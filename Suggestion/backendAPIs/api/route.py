...
#API to fetch MUSIC by title from database
@api.route('/api/music', methods=["GET"])
@jwt_required(locations=["headers"])
def musiclist():
    if not current_user:
        return jsonify({"message": "Login required"})
    music = []
    response = list(db1.music_collection.find())
    if response:
        music_fetched = response
        for music in music_fetched:
            music["_id"] = str(music["_id"])
            music.append(music)
    else:
        pass
    selected_books = music[:1000]
    return jsonify(selected_books)
