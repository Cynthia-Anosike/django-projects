...
export default function Movies() {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await axios.get('http://127.0.0.1:8080/api/movies', options);
            const apiData = response.data;
            const shuffleData = shuffleArray(apiData)
            setData(shuffleData);
            setLoading(false);
            console.log(apiData);
        } catch (error) {
    ...
    const groupedData = data.reduce((groups, item) => {
        const category = item.genre; // Assuming categories is an array
        if (!groups[category]) {
            groups[category] = [];
        }
    ...
    // const filteredUniqueItems = (items) => {
    ...
    const displayData = Object.entries(groupedData).map(([category, items]) => {
        ...
        const categoryFilteredItems = items.filter((item) => {
            ...
            const categories = item.genre.toString();
            ...
            <div key={category} className="my-8 ">
                ...
                <ul className="grid gap-4 grid-cols-2 md:grid-cols-3 lg:grid-cols-4 pr-5">
                    {combinedFilteredItems.map((item) => (
                        <li key={item.id} className="border border-[#cfbb46] rounded overflow-hidden hover:shadow-lg " onClick={() => openModal(item)}>
                            <img
                                src={init + item.posterURL}
                                alt=""
                                className="w-full h-[15rem] object-cover transition-transform transform hover:scale-105 ease-in-out"
                            />
                            <div className="p-3">
    ...
    const displayGenre = categories.map((id) => {
        return genreMap[id] || id
    })

    const categoryList = (
        <select onChange={handleTest} className="rounded border-2  border-cyan-500 sm:mb-0 py-2.5 ps-8 pe-8 shadow-sm text-sm  bg-[#03010bcb] outline-0 w-[90vw] md:w-auto lg:w-auto text-gray-200">
            <option value=''>List All Genres</option>
    ...
    return (
        <div className="bg-[#03010bcb]">
            {loading ?
                <div className="h-[14vh] flex flex-col gap-1 lg:flex-row md:flex-row w-full border-b-2 justify-center items-center fixed z-[999] top-0 mb-[50px] pb-2 md:pb-0 lg:pb-0" >
                ...
                : (
                    ...
                    <div className=" flex flex-col gap-0 lg:flex-row md:flex-row w-full border-b-[1px] justify-center items-center fixed z-[999] bg-[rgb(37,33,41)] top-0 mb-[50px] pb-2 md:pb-0 lg:pb-0" >
            ...
            <div className="overflow-y-auto pt-[150px] md:pt-[100px] lg:pt-[100px]">
                {
                    loading ? <Skeleton />
                        :
                        (displayData)
